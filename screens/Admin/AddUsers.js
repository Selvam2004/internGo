...
export default function AddUsers() { 
  ...
  const handleSave = async () => { 
    if (validate()) {
      ...
      try {
        ...
        const message = err?.response?.data?.message || "User not added ❌";
  ...
  const validate = () => {
    ...
    if (!/^[a-zA-Z0-9._%+-]+@(?:finestcoder\.com|codingmart\.com)$/.test(fields.email)) {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.userContainer}> 
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput style={styles.input} placeholder='Enter email' onChangeText={(text) => handleChange('email', text)} value={fields.email} />
        </View>

        <View style={styles.inputContainer}>
          <Text style={styles.label}>Password:</Text>
          <TextInput style={styles.input} placeholder='Enter password' secureTextEntry={true} onChangeText={(text) => handleChange('password', text)} value={fields.password} />
        </View>
